...
void imc_printf( D_MOBILE *ch, const char *fmt, ... ) __attribute__ ( ( format( printf, 2, 3 ) ) );
void imcpager_printf( D_MOBILE *ch, const char *fmt, ... ) __attribute__ ( ( format( printf, 2, 3 ) ) );
char *imc_funcname( IMC_FUN *func );
IMC_FUN *imc_function( const char *func );
char *imc_send_social( D_MOBILE *ch, char *argument, int telloption );
...
char *color_itom( const char *txt, D_MOBILE *ch )
{
   IMC_COLOR *color;
   static char tbuf[LGST];
...
void imc_to_char( const char *txt, D_MOBILE *ch )
{
   char buf[LGST*2];

   snprintf( buf, LGST*2, "%s\033[0m", color_itom( txt, ch ) );
   text_to_mobile( ch, buf );
   return;
}

/* Modified version of Smaug's ch_printf_color function */
void imc_printf( D_MOBILE *ch, const char *fmt, ... )
{
   char buf[LGST];
   va_list args;
...
void imc_to_pager( const char *txt, D_MOBILE *ch )
{
   char buf[LGST*2];
...
void imcpager_printf( D_MOBILE *ch, const char *fmt, ... )
{
   char buf[LGST];
   va_list args;
...
/*
 * Returns a D_MOBILE class which matches the string
 */
D_MOBILE *imc_find_user( char *name ) 
{
   D_SOCKET *d;
   ITERATOR *pIter;
   D_MOBILE *vch = NULL;
   ...
   while( ( d = (D_SOCKET *)NextInList( pIter ) ) != NULL )
   {
	if( ( vch = d->player ) != NULL && !strcasecmp( CH_IMCNAME(vch), name )
...
char *imcgetname( D_MOBILE *ch, char *from )
{
...
bool imc_isignoring( D_MOBILE *ch, const char *ignore )
{
   IMC_IGNORE *temp;
...
bool check_mud( D_MOBILE *ch, char *mud )
{
...
bool check_mudof( D_MOBILE *ch, char *mud )
{
...
bool imccheck_permissions( D_MOBILE *ch, int checkvalue, int targetvalue, bool enforceequal )
{
...
void imcformat_channel( D_MOBILE *ch, IMC_CHANNEL *d, int format, bool all )
{
   IMC_CHANNEL *c = NULL;
   char buf[LGST];
...
void imc_update_tellhistory( D_MOBILE *ch, const char *msg )
{
   char new_msg[LGST];
...
PFUN( imc_recv_emote )
{
   ...
   while( ( d = (D_SOCKET *)NextInList( pIter ) ) != NULL )
   {
      if( d->state == STATE_PLAYING && ( ch = d->player ) != NULL
       && IMCPERM(ch) >= level )
...
void imc_sendnotify( D_MOBILE *ch, char *chan, bool chon )
{
   IMC_PACKET *p;
   IMC_CHANNEL *channel;
...
char *imcrankbuffer( D_MOBILE *ch )
{
...
char *imc_assemble_who( void )
{
   ...
   while( ( d = (D_SOCKET *)NextInList( pIter ) ) != NULL )
   {
      ...
      if( person && d->state == STATE_PLAYING )
      {
...
void imc_adjust_perms( D_MOBILE *ch )
{
   if( !this_imcmud )
	return;
...
void imc_char_login( D_MOBILE *ch )
{
   if( !this_imcmud )
	return;
...
bool imc_loadchar( D_MOBILE *ch, FILE *fp, const char *word )
{
   bool fMatch = FALSE;
...
void imc_savechar( D_MOBILE *ch, FILE *fp )
{
   IMC_IGNORE *temp;
...
void imc_freechardata( D_MOBILE *ch )
{
...
IMC_FUN *imc_function( const char *func )
{
   ...
   if( !strcasecmp( func, "imctemplates" ) )                return imctemplates;

   return NULL;
}

/* Check for IMC channels, return TRUE to stop command processing, FALSE otherwise */
bool imc_command_hook( D_MOBILE *ch, char *command, char *argument )
{
   IMC_CMD_DATA *cmd;
   IMC_ALIAS *alias;
   IMC_CHANNEL *c;
   ...
   switch( argument[0] )
   {
	case ',':
	   ...
	   break;
	case '@':
         imc_to_char( "Sorry, no social system exists for this codebase yet.\n\r", ch );
	   break;
	default:
